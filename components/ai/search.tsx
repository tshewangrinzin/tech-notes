...
function Message({
  ...
  for (const part of message.parts ?? []) {
    if (part.type === "text") {
      const visible = getVisibleText(part.text);
      markdown += `${markdown && visible ? "\n\n" : ""}${visible}`;
      continue;
    }
  ...
  return (
    <div className="space-y-1" {...props}>
      ...
      <div
        className={cn(
          "rounded-2xl border px-3.5 py-3 text-sm shadow-sm",
          message.role === "assistant"
            ? "bg-fd-card border-fd-border text-fd-card-foreground"
            : "bg-fd-primary/8 border-fd-primary/20 text-fd-foreground",
        )}
      >
        <div className="prose prose-sm max-w-none leading-6 prose-headings:scroll-mt-24 prose-p:my-2 prose-ul:my-2 prose-ol:my-2 prose-li:my-0.5">
...
export function AISearchTrigger({
  ...
  return (
    <button
      ...
      className={cn(
        position === "float" && [
          "fixed bottom-4 gap-3 w-24 end-[calc(--spacing(4)+var(--removed-body-scroll-bar-size,0px))] shadow-lg z-40 transition-[translate,opacity]",
        ...
        <div
          className={cn(
            ...
            mode === "docs"
              ...
              : "lg:fixed lg:end-4 lg:top-4 lg:bottom-4 lg:border lg:rounded-2xl lg:shadow-xl",
            open
